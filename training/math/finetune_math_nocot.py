"""
Fine-tuning script — Hendrycks MATH No-CoT
============================================
...
OUTPUT_DIR   = "lora_output_math_nocot_[model_tag]"
DATASET_MODE = "math_nocot"
...
print(f"Mode: MATH-No-CoT | Seed: {SEED} | GPU: {_gpu_name}")
...
def make_nocot_response(example):
    """No-CoT: final boxed answer only, no reasoning trace."""
...
def main():
    ...
    train_ds = train_ds.map(
        ...
        num_proc=1,
        desc="Format MATH train (No-CoT)",
    )
    test_ds = test_ds.map(
        lambda x: format_sample(x, tokenizer),
        remove_columns=test_ds.column_names,
        num_proc=1,
        desc="Format MATH eval (No-CoT)",
    )
