"""
Fine-tuning script — TIGER-Lab/MathInstruct (Chain-of-Thought)
===============================================================
...
CoT response format:
  Full output field (reasoning steps already present)
  FINAL ANSWER: <extracted answer>

Fields in dataset: instruction (problem), output (solution), source (origin)

NOTE on PoT entries:
  ~40% of MathInstruct entries are Program-of-Thought (Python code solutions).
  We include ALL entries (CoT + PoT) in CoT fine-tuning since both contain
  step-by-step reasoning. The `source` field encodes CoT vs PoT.
...
OUTPUT_DIR        = "lora_output_mathinstruct_cot_[model_tag]"
DATASET_MODE      = "mathinstruct_cot_[model_tag]"
...
print(f"Mode: MathInstruct-CoT ([BASE_MODEL_NAME]) | Seed: {SEED} | GPU: {_gpu_name}")
...
def make_cot_response(example):
    """
    CoT: full output (reasoning trace already present in MathInstruct)
    followed by FINAL ANSWER marker.
    """
...
def main():
    ...
    # Shuffle with fixed seed — must match nocot + eval scripts
