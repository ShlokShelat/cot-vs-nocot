#!/bin/bash
#SBATCH --job-name=dfa_cot
#SBATCH --output=/home/[USER]/cot_vs_nocot/new_dataset_nfa/logs/dfa_cot_%j.out
#SBATCH --error=/home/[USER]/cot_vs_nocot/new_dataset_nfa/logs/dfa_cot_%j.err
...
mkdir -p $BASE/logs $BASE/lora_output_dfa_cot $BASE/eval_results
...
if [ ! -f "$SENTINEL" ]; then
    echo "========================================================"
    echo "Generating DFA parallel dataset..."
    echo "========================================================"
    ...
    echo "DFA dataset already exists, skipping generation."
fi

# ── STEP 2: CoT fine-tuning ───────────────────────────────────────────────────
echo "========================================================"
echo "DFA CoT fine-tuning..."
echo "========================================================"
python3 -u finetune_dfa_cot.py
...
python3 -u evaluate_dfa.py \
    --model_path /path/to/[MODEL_NAME] \
    --lora_path  lora_output_dfa_cot \
    --data_file  dfa_cot_ood_test.jsonl \
    --mode       cot \
    --ood \
    --out        eval_results/dfa_cot_ood.json
echo "CoT OOD done: $(date)"

# ── SUMMARY ──────────────────────────────────────────────────────────────────
echo "========================================================"
echo "SUMMARY (DFA CoT job)"
echo "========================================================"
python3 -u - << 'PYEOF'
import json, glob, os
